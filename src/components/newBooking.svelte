<script lang="ts">
  ...
  <div class="p-4 pt-8 mb-4 mt-4 shadow-md rounded-lg border-[1px] border-borderColor">
      ...
      <div class="border-[1px] h-28 overflow-auto select-none border-borderColor divide-borderColor rounded-lg shadow-sm divide-y" style="height: 7.8rem;">
      ...
      <form on:submit={createBooking}>
        <div class="py-2">
            ...
            {#if selectedDate}
              <input class="border-borderColor border-[1px] rounded-md px-2 py-2 mt-1 shadow-sm min-w-full" type="hidden" id="date" name="date" value="{formatDate(selectedDate, '-')}" readonly={true}/>
            <!-- {:else}
